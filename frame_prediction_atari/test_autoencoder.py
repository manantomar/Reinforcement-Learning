    ...
    while num_episodes < MAX_EPISODES:
        ...
        for t in range(10000):
            ...
            if num_episodes > 32:
                ...
                pred_batch = encode.eval(feed_dict = {action : np.reshape(action_batch, (BATCH, 6)), state : np.reshape(state_batch, (BATCH, 84, 84, 4))})
                ...
                loss = cost.eval(feed_dict = {y : target_batch,
                        ...
                        state : state_batch,
                        action : action_batch})
                summary = merged_summary_op.eval(feed_dict = {y : target_batch,
                        pred_frame : pred_batch,
                        state : state_batch,
                        action : action_batch})
