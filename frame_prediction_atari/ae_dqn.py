...
# redundant as for now
#flags = tf.app.flags
#flags.DEFINE_boolean('train', True, 'Whether to do training or testing')
#flags.DEFINE_string('env_name', 'PongNoFrameskip-v0', 'The name of gym environment to use')

def parse_args():
    parser = argparse.ArgumentParser("Run an already learned DQN model.")
    # Environment
    parser.add_argument("--env", type=str, required=True, help="name of the game")
    parser.add_argument("--is_train", default=True, help="name of the game")
...
class autoencoder():
    ...
    def update(self, sess, s, a, y, p, global_step):
        """
...
def rollout(sess, prediction_net, act):


    #tf.summary.scalar("Qval", encode)
    merged_summary_op = tf.summary.merge_all()

    sess.run(tf.variables_initializer(var_list = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope = "ae")))
    saver = tf.train.Saver(var_list = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope = "ae"))
    print("ae varibles initialized and saver defined")
    ...
    while num_episodes < MAX_EPISODES:
        s_t = env.reset()
        #ob = env.reset()
        #print("shape obf", np.asarray(obf).shape)
        ...
        for t in range(10000):
            ...
            action_id = act(np.array(s_t)[None], stochastic=args.stochastic)[0]
            action_vector = np.zeros(ACTIONS)
            action_vector[action_id] = 1
            #actions.append(action_vector)

            s_t1, reward, done, info = env.step(action_id)
            ...
            if args.is_train:
                ...
                D.append((np.array(s_t) / 255.0, action_vector, np.array(s_t1)[:, :, 3] / 255.0))
                ...
                if num_episodes > 2:
                    ...
                    loss = prediction_net.update(sess, state_batch, action_batch, target_batch, pred_batch, k)
                ...
                prediction = prediction_net.predict(sess, np.reshape(np.asarray(s_t), (1, 84, 84, 4)), np.reshape(action_vector, (1, 6)))
...
with tf.Session() as sess:
    ...
    #U.load_state('/tmp/models/model-atari-pong-1/saved')
    prediction_net = autoencoder("ae")
    rollout(sess, prediction_net, act)
